...
try {
	...
	if (strlen($_POST["edit_phone_number"]) != 10) {
		$_SESSION["error_msg"] .= "<br>Numero muodossa: XXXXXXXXXX, esim: 0501234567";
		$virhe = true;
	}
	...
	$user_query = create_connection()->prepare("UPDATE Users SET first_name = ?, last_name = ?, gender = ?, country = ?, city = ?,
						 description = ?, phone_number = ?, email = ?, sex_pref = ?, relationship_type = ? WHERE user_id = ?");

	$user_query->execute(array($_POST["edit_first_name"], $_POST["edit_last_name"], $_POST["edit_gender"], $_POST["edit_country"], 
				$_POST["edit_city"], $_POST["edit_description"], $_POST["edit_phone_number"], $_POST["edit_email"], 
				$_POST["edit_sex_pref"], $_POST["sex_pref"], $user->user_id));
